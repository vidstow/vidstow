...
test('page titles and controls match the approved redesign', async () => {
  ...
  assert.match(home, /grid-template-columns: 160px minmax\(0, 1fr\);/);
  assert.match(home, /grid-template-areas: 'thumb identity'/);
  assert.doesNotMatch(home, /'thumb identity format'/);
  assert.doesNotMatch(home, /'format format'/);
  assert.doesNotMatch(home, /class="dformat"/);
  ...
  assert.doesNotMatch(home, /aria-label="Format"/);
