...
describe('Downloads format truthfulness', () => {
  test('prefers the completed filename extension and reports fallback delivery', async () => {
    history.set([{
      ...
      deliveryNote: 'Saved as MKV with an SRT subtitle sidecar after embedding fallback.',
    }]);
    ...
    expect(screen.getByText('Saved as MKV with an SRT subtitle sidecar after embedding fallback.')).toBeInTheDocument();
  });
});
