<script lang="ts">
  ...
  const activeCollections = $derived(collections.filter((collection) =>
    collection.childJobIds.length === 0 || collection.childJobIds.some((id) => jobsById.get(id)?.lifecycle !== 'completed'),
  ));
  const standaloneJobs = $derived(activeJobs.filter((job) => !job.collectionId || !collectionIds.has(job.collectionId)));
  ...
  function collectionChildren(childJobIds: string[]) {
    return childJobIds.map((id) => jobsById.get(id)).filter((job): job is LifecycleJobViewModel => job !== undefined && job.lifecycle !== 'completed');
  }
...
<section class="queue-shell" aria-labelledby="lifecycle-queue-title">
  <div class="queue-master">
    ...
    <div class="queue-list">
      {#if activeJobs.length || activeCollections.length}
        {#each activeCollections as collection (collection.id)}
          <CollectionRow {collection} children={collectionChildren(collection.childJobIds)} selectedJobId={selectedJob?.id} onSelect={(id) => selectedJobId = id} onAction={forward} onCollectionAction={forwardCollection} />
        {/each}
        {#if standaloneJobs.length}
          <ul class="standalone-list" aria-label="Queue items">
            {#each standaloneJobs as job, index (job.id)}
              <LifecycleJobRow {job} index={index + 1} selected={selectedJob?.id === job.id} onSelect={(id) => selectedJobId = id} onAction={forward} />
            {/each}
          </ul>
        {/if}
